...
class JobGenerator:
    ...
    def __init__(self, nb_steps, n_window, n_on_premise_node, n_cloud_node, config):
        ...
        self.nb_episodes = self.config['param_simulation']['nb_episodes']
    ...
    def generate_jobs_set(self):
        if self.job_type == 1:
            jobs = np.array(
                  # [submit_time, processing_time, required_nodes_num, can_use_cloud,user_id, job_id, waiting_time(初期値=-1)]
[[0, 1, 3, 1, 0, 0, -1, -1],
 [0, 1, 3, 1, 0, 1, -1, -1],
 [0, 1, 3, 1, 0, 2, -1, -1],
 [0, 1, 3, 1, 0, 3, -1, -1],
