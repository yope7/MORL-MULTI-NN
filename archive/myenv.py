...
class SchedulingEnv(gym.core.Env):
    def __init__(self, max_step, n_window, n_on_premise_node, n_cloud_node, n_job_queue_obs, n_job_queue_bck, weight_wt,
        ...
        self.on_premise_window_history = [] # オンプレミスのスライドウィンドウの履歴
        self.cloud_window_history = [] # クラウドのスライドウィンドウの履歴
    ...
    def step(self, action_raw):
        ...
        # 報酬を取得
        reward = self.get_reward(action, allocated_job, time, is_valid)
        # コストを取得
        ...
        return observation, reward, cost, time, done, info
    ...
    def reset(self):
        ...
        self.on_premise_window = np.zeros((self.n_on_premise_node, self.n_window))  # オンプレミスのスライドウィンドウ
        self.cloud_window = np.zeros((self.n_cloud_node, self.n_window))  # クラウドのスライドウィンドウ
        self.job_queue = np.zeros((self.n_job_queue_obs + self.n_job_queue_bck, 7))  # ジョブキュー
        self.rear_job_queue = 0  # ジョブキューの末尾 (== 0: ジョブキューが空)
        self.tmp_queue = deque()  # 割り当てられなかったジョブを一時的に格納するキュー
    ...
    # オンプレミスウィンドウの最初の列を履歴に追加
        if len(self.on_premise_window_history) == 0:
            self.on_premise_window_history = self.on_premise_window[:, 0].reshape(-1, 1)
        else:
            self.on_premise_window_history = np.hstack((self.on_premise_window_history, self.on_premise_window[:, 0].reshape(-1, 1)))

        # クラウドウィンドウの最初の列を履歴に追加
        if len(self.cloud_window_history) == 0:
            self.cloud_window_history = self.cloud_window[:, 0].reshape(-1, 1)
        else:
            self.cloud_window_history = np.hstack((self.cloud_window_history, self.cloud_window[:, 0].reshape(-1, 1)))
        if np.all(self.job_queue[0] == 0):  # 先頭ジョブが空(ジョブキューが空)
            #             print('先頭ジョブが空(ジョブキューが空)')
            # 時間を1進める
                    # オンプレミスウィンドウとクラウドウィンドウの状態を保存
    ...
    # エピソード終了時にウィンドウの履歴を取得
    def get_window_history(self):
        return self.on_premise_window_history, self.cloud_window_history

    # エピソード終了時にウィンドウの履歴をリセット
    def reset_window_history(self):
        self.on_premise_window_history = []
        self.cloud_window_history = []
    # ジョブキューに新しいジョブを追加
    ...
    # 報酬を取得
    def get_reward(self, action, allocated_job, time, is_valid):
        reward_liner = 0
        reward_wt = 0
        reward_cost = 0
        reward = [0,0]
        if is_valid:  # actionが有効だった場合
            ...
            if when_allocate != -1:  # 割り当てた場合
                ...
                reward_wt = -estimated_waiting_time
                reward_cost = 0

        else:  # actionが無効だった場合
            reward_liner = -self.penalty_invalid_action
            reward_wt = -self.penalty_invalid_action
            reward_cost = -self.penalty_invalid_action
