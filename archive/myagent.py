...
class DQNAgent_without_action_filter(DQNAgent):
    def __init__(self, model_cloud_cost,model_waiting_time, n_window, n_on_premise_node, n_cloud_node, n_job_queue_obs, n_job_queue_bck, policy=None,
    ...
    def fit(self, env, nb_steps, action_repetition=1, callbacks=None, verbose=1,
            visualize=False, nb_max_start_steps=0, stacompute_q_valuesrt_step_policy=None, log_interval=10000,
        ...
        try:
            while self.step < nb_steps:
                ...
                for _ in range(action_repetition): #再配置するならここで呼び出す？NY
                    callbacks.on_action_begin(action)
                ...
                if done:
                    ...
                    #ハイパーボリュームを求める.HVは最大化せよ$$$$$
                    result_vector.append(reward_record)
                    hypervolume = get_hypervolume(result_vector)
                    hypervolume = 1
                    self.backward(hypervolume, terminal=False)
...
from myagent import DQNAgent

class MultiObjectiveDQNAgent(DQNAgent):
    def __init__(self, model_waiting_time, model_cloud_cost, *args, **kwargs):
        super().__init__(model=model_waiting_time, *args, **kwargs)
        self.model_waiting_time = model_waiting_time
        self.model_cloud_cost = model_cloud_cost

    def forward(self, observation):
        # 両方のモデルからQ値を取得
        q_values_waiting_time = self.model_waiting_time.predict(observation)
        q_values_cloud_cost = self.model_cloud_cost.predict(observation)

        # Q値の組み合わせ方にはいくつかのアプローチがあります（例：平均、重み付け）
        combined_q_values = (q_values_waiting_time + q_values_cloud_cost) / 2

        # 選択されたアクション
        action = self.policy.select_action(combined_q_values)
        return action

    def backward(self, reward, terminal):
        # 報酬を分離
        reward_waiting_time, reward_cloud_cost = reward

        # 待ち時間用のモデルを更新
        self.model_waiting_time.train_on_batch(observation, reward_waiting_time, terminal)

        # クラウドコスト用のモデルを更新
        self.model_cloud_cost.train_on_batch(observation, reward_cloud_cost, terminal)

        # 元のbackwardメソッドを呼び出す
        return super(MultiObjectiveDQNAgent, self).backward(reward, terminal)
