...
class MyAgent(DQNAgent):
    ...
    def get_hypervolume(self, vector):
        #calculate hypervolume
        #点の集合を受け取り、そのhypervolumeを返す
        #原点からの長方形の面積を、それぞれ重なった点がある場合は重複分を引いて計算する
        hv = 0
        for i in range(len(vector)):
            for j in range(i+1,len(vector)):
                hv += (vector[i][0] - vector[j][0]) * (vector[i][1] - vector[j][1])
            return hv
            ...
            while self.step < nb_steps:
                if observation is None:  # start of a new episode
                    ...
                    for _ in range(nb_random_start_steps):
                        ...
                        reward_record = [sum(x) for x in zip(reward_record, reward)] 
                ...
                metrics = self.backward(1, terminal=done) #$$$$
                ...
                if done:
                    ...
                    hypervolume = self.get_hypervolume(result_vector)
                ...
                if observation is None:  # start of a new episode
                    ...
                    for _ in range(nb_random_start_steps):
                        ...
                        if self.processor is not None:
                            action = self.processor.process_action(action)
                        callbacks.on_action_begin(action)
                        observation, reward1, cost, time, done, info = env.step(action)
                        observation = deepcopy(observation)
                        if self.processor is not None:
                            observation, reward1, done, info = self.processor.process_step(
                                observation, reward1, done, info)
                        callbacks.on_action_end(action)
                        if done:
                            warnings.warn(
                                'Env ended before {} random steps could be performed at the start. You should probably lower the `nb_max_start_steps` parameter.'.format(
