...
class JobGenerator:
    ...
    def __init__(self, nb_steps, n_window, n_on_premise_node, n_cloud_node, config):
        self.config = config
        self.nb_steps = nb_steps
        # 最大時間を入力
        self.max_t = self.config['param_job']['max_t']
        self.nb_episodes = nb_steps // self.max_t
    ...
    def generate_jobs_set(self):
        ...
        # ランダムの場合，必要な変数を指定
            ...
            prm_job = [self.nb_steps, self.max_t, n_job_per_time, max_processing_time, max_n_required_nodes]
            self.df_prmj = pd.DataFrame(
                [prm_job],
                columns=['n_steps', 'max_time', 'n_job_per_time', 'max_processing_time', 'max_n_required_nodes'],
                index=['idx']
            )
    ...
    # ジョブ単体を生成
    def generate_job(self, submit_time, processing_time, n_required_nodes, can_use_cloud, waiting_time=-1):
        job = [submit_time, processing_time, n_required_nodes, can_use_cloud, self.job_id, waiting_time]
        self.job_id += 1
    ...
    def save_jobs_set(self, path):
        with open(path, 'w') as f:
            json.dump(self.jobs_set, f, indent=4)